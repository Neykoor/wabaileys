...
export class USyncDeviceProtocol {
    ...
    getUserElement( /* user: USyncUser */) {
        // Device phashing (ts/expectedTs) is optional; when absent WA returns all devices for the user.
        // Returning null here is correct: the query node is omitted and WA falls back to full device list.
        return null;
    }
    parser(node) {
        const deviceList = [];
