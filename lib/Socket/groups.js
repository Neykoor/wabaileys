...
export const makeGroupsSocket = (config) => {
    ...
    const { groupMetadataCacheMs } = config;
    ...
    const _groupMetadataInflight = new Map();
    const _groupMetadataCache = groupMetadataCacheMs > 0
        ? new LRUCache({ max: 500, ttl: groupMetadataCacheMs, ttlAutopurge: true })
        : null;
    const groupMetadata = async (jid) => {
        if (_groupMetadataCache) {
            const cached = _groupMetadataCache.get(jid);
            if (cached)
                return cached;
        }
        const inflight = _groupMetadataInflight.get(jid);
        if (inflight)
            return inflight;
        const promise = (async () => {
            const result = await groupQuery(jid, 'get', [{ tag: 'query', attrs: { request: 'interactive' } }]);
            const meta = extractGroupMetadata(result);
            _groupMetadataCache?.set(jid, meta);
            return meta;
        })().finally(() => _groupMetadataInflight.delete(jid));
        _groupMetadataInflight.set(jid, promise);
        return promise;
    };
    if (_groupMetadataCache) {
        // Invalidate immediately on real changes instead of waiting out the TTL,
        // so a promote/demote/add/remove is reflected on the very next call.
        ev.on('group-participants.update', ({ id }) => _groupMetadataCache.delete(id));
        ev.on('groups.update', (updates) => {
            for (const u of updates) {
                if (u.id)
                    _groupMetadataCache.delete(u.id);
            }
        });
    }
    ...
    const groupFetchAllParticipating = async () => {
        ...
        if (groupsChild) {
            ...
            for (const groupNode of groups) {
                ...
                data[meta.id] = meta;
                _groupMetadataCache?.set(meta.id, meta);

                for (const participant of meta.participants) {
                    if (participant.lid && participant.phoneNumber) {
                        lidMappingPairs.push({ lid: participant.lid, pn: participant.phoneNumber });
                    } else if (participant.lid && participant.id && isPnUser(participant.id)) {
                        lidMappingPairs.push({ lid: participant.lid, pn: participant.id });
                    } else if (isLidUser(participant.id) && participant.phoneNumber) {
                        lidMappingPairs.push({ lid: participant.id, pn: participant.phoneNumber });
                    }
                }
            }

            if (lidMappingPairs.length > 0 && sock.signalRepository?.lidMapping) {
                await sock.signalRepository.lidMapping.storeLIDPNMappings(lidMappingPairs).catch(err => {
                    config.logger?.warn({ err }, 'WAbaileys: failed to store LID-PN mappings from group fetch');
                });
            }
        }
...
export const extractGroupMetadata = (result) => {
    ...
    return {
        ...
        size: group.attrs.size ? +group.attrs.size : participants.length,
