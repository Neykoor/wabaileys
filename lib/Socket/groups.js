...
export const makeGroupsSocket = (config) => {
    ...
    const groupFetchAllParticipating = async () => {
        ...
        if (groupsChild) {
            ...
            if (lidMappingPairs.length > 0 && sock.lidMapping) {
                await sock.lidMapping.storeLIDPNMappings(lidMappingPairs).catch(err => {
...
export const extractGroupMetadata = (result) => {
    ...
    const participants = getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
        ...
        const phoneNumber = isLidUser(participantJid) && isPnUser(attrs.phone_number)
            ? attrs.phone_number
            : undefined;
        const lid = isPnUser(participantJid) && isLidUser(attrs.lid)
            ? attrs.lid
            : isLidUser(participantJid)
                ? participantJid
                : undefined;
