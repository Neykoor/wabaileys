...
export const makeMessagesRecvSocket = (config) => {
                        ...
                        try {
                            ...
                            const fullMessage = proto.WebMessageInfo.fromObject({
                                key: {
                                    ...
                                    fromMe: jidNormalizedUser(from) === jidNormalizedUser(authState.creds.me?.id)
                                },
    ...
    const reissueTcTokenAfterIdentityChange = (from) => {
        void (async () => {
            ...
            const [tcTokenData, issueJid] = await Promise.all([
                authState.keys.get('tctoken', [tcJid]),
    ...
    const handleGroupNotification = (fullNode, child, msg) => {
        ...
        switch (child?.tag) {
            ...
            case 'leave':
                ...
                const lidPnPairs = participants
                    .filter(p => isPnUser(p.id) && p.lid)
                    .map(p => ({ lid: p.lid, pn: p.id }));
                const lidPhonePairs = participants
                    .filter(p => isLidUser(p.id) && p.phoneNumber)
                    .map(p => ({ lid: p.id, pn: p.phoneNumber }));
                const allPairs = [...lidPnPairs, ...lidPhonePairs];
                if (allPairs.length > 0) {
                    signalRepository.lidMapping.storeLIDPNMappings(allPairs)
                        .catch(err => logger.warn({ err }, 'failed to store LID mappings from group participant event'));
                }
            ...
            case 'revoked_membership_requests':
                ...
                if (affectedParticipantLid && affectedParticipantPn) {
                    const pair = isPnUser(affectedParticipantPn) && isLidUser(affectedParticipantLid)
                        ? [{ lid: affectedParticipantLid, pn: affectedParticipantPn }]
                        : [];
                    if (pair.length > 0) {
                        signalRepository.lidMapping.storeLIDPNMappings(pair)
                            .catch(err => logger.warn({ err }, 'failed to store LID mapping from revoked_membership_requests'));
                    }
                }
                msg.messageStubType = WAMessageStubType.GROUP_MEMBERSHIP_JOIN_APPROVAL_REQUEST_NON_ADMIN_ADD;
                msg.messageStubParameters = [
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'mex':
                ...
                break;
            case 'w:gp2': {
                const participantLid = node.attrs.participant;
                const participantPn = node.attrs.participant_pn;
                if (participantLid && participantPn && isLidUser(participantLid) && isPnUser(participantPn)) {
                    signalRepository.lidMapping.storeLIDPNMappings([{ lid: participantLid, pn: participantPn }])
                        .catch(err => logger.warn({ err }, 'failed to store LID mapping from w:gp2 notification'));
                }
            ...
            case 'picture':
                ...
                const pictureJid = jidNormalizedUser(node?.attrs?.from)
                    || (pictureNode?.attrs?.id ? jidNormalizedUser(pictureNode.attrs.id) : undefined)
                    || pictureNode?.attrs?.hash
                    || '';
    ...
    const handleMessage = async (node) => {
        ...
        // msmsg (message secret) encryption is not supported; send NACK so WA does not retry
        if (encNode?.attrs.type === 'msmsg') {
            logger.debug({ key: node.attrs.key }, 'ignored msmsg - message secret encryption not supported');
                ...
                if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT && msg.category !== 'peer') {
                    ...
                    if (!isNewsletter) {
                        ...
                        if (category === 'peer') {
                            ...
                            type = 'sender';
                            // receipts must go to the PN, not the LID - resolve via lidMapping if available
                            if (isLidUser(msg.key.remoteJid) || isLidUser(msg.key.remoteJidAlt)) {
                                const resolvedPn = author
                                    ? await signalRepository.lidMapping.getPNFromLID(author).catch(() => null)
                                    : null;
                                participant = resolvedPn || author;
                            }
                        }
            ...
            if (attrs.error === SERVER_ERROR_CODES.MessageAccountRestriction) {
                ...
                if (ackFrom && !inFlight463Recoveries.has(ackFrom)) {
                    ...
                    void (async () => {
                        try {
                            const getPNForLID = signalRepository.lidMapping.getPNForLID.bind(signalRepository.lidMapping);
                            const [tcStorageJid, issueJid] = await Promise.all([
                                resolveTcTokenJid(ackFrom, getLIDForPN),
                                resolveIssuanceJid(ackFrom, sock.serverProps.lidTrustedTokenIssueToLid, getLIDForPN, getPNForLID)
                            ]);
