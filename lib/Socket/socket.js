...
import { MexQueryIds, MexXWAPaths, ReachoutTimelockEnforcementType, DisconnectReason } from '../Types/index.js';
import { addTransactionCapability, aesEncryptCTR, bindWaitForConnectionUpdate, buildPairingQRData, bytesToCrockford, configureSuccessfulPairing, Curve, derivePairingCodeKey, generateLoginNode, generateMdTagPrefix, generateRegistrationNode, getCodeFromWSError, getCompanionPlatformId, getErrorCodeFromStreamError, getNextPreKeysNode, makeEventBuffer, makeLidUtility, makeNoiseHandler, promiseTimeout, signedKeyPair, xmppSignedPreKey } from '../Utils/index.js';
...
export const makeSocket = (config) => {
    ...
    if (printQRInTerminal) {
        logger.warn({}, '⚠️ printQRInTerminal is deprecated. Listen to connection.update for the QR instead, then remove this option.');
    }
    ...
    const query = async (node, timeoutMs) => {
        ...
        await sendNode(node);
        const result = await waitForMessage(msgId, timeoutMs);
    ...
    // 🛠️ FIX: contraparte de pnFromLIDUSync — resuelve número real a partir de un LID.
    // Antes no existía, por eso getPNsForLIDs (lid-mapping.js) nunca podía resolver
    // por red y dependía 100% de que la mapping ya estuviera cacheada/guardada localmente.
    const lidToPnUSync = async (lids) => {
        const usyncQuery = new USyncQuery().withLIDProtocol().withContext('background');
        for (const lid of lids) {
            if (!isLidUser(lid)) {
                continue;
            }
            usyncQuery.withUser(new USyncUser().withLid(lid));
        }
        if (usyncQuery.users.length === 0) {
            return [];
        }
        const results = await executeUSyncQuery(usyncQuery);
        if (results) {
            // El servidor devuelve el par completo (id = pn, lid = lid) igual que en la
            // consulta directa; filtramos por los que sí trajeron ambos valores.
            return results.list
                .filter(a => !!a.lid && !!a.id)
    ...
    const signalRepository = makeSignalRepository({ creds, keys }, logger, pnFromLIDUSync, lidToPnUSync);
    let lastDateRecv;
    let epoch = 1;
    let keepAliveReq;
    let qrTimer;
    ...
    const uploadPreKeysToServerIfRequired = async () => {
        try {
            const [preKeyCount, { exists: currentPreKeyExists, currentPreKeyId }] = await Promise.all([
                getAvailablePreKeysOnServer(),
                verifyCurrentPreKeyExists()
            ]);
    ...
    ws.on('CB:success', async (node) => {
        ...
        void (async () => {
            if (!node.attrs.lid || !authState.creds.me?.id) return;
    ...
    const fetchAccountReachoutTimelock = async () => {
        const queryResult = await executeWMexQuery({}, MexQueryIds.REACHOUT_TIMELOCK, MexXWAPaths.xwa2_fetch_account_reachout_timelock, query, generateMessageTag);
        const result = {
    ...
    const fetchNewChatMessageCap = async () => {
        return executeWMexQuery({ input: { type: 'INDIVIDUAL_NEW_CHAT_MSG' } }, MexQueryIds.MESSAGE_CAPPING_INFO, MexXWAPaths.xwa2_message_capping_info, query, generateMessageTag);
    };
    return {
        type: 'md',
        ws,
        ...
        signalRepository,
        lid: makeLidUtility(signalRepository.lidMapping),
        get user() {
