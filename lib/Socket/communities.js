...
export const makeCommunitiesSocket = (config) => {
    ...
    const { authState, ev, query, upsertMessage, relayMessage, waUploadToServer } = sock;
    ...
    // Interactive message handler (payment, product, interactive, album, event,
    // poll result, group story, status). Built here because relayMessage and
    // waUploadToServer are only available once the messages-send mixin has run.
    const dugong = new Dugong(waUploadToServer, relayMessage, config, sock);
    return {
        ...sock,
        dugong,
        communityQuery,
        ...
        communityAcceptInviteV4: ev.createBufferedFunction(async (key, inviteMessage) => {
            ...
            await upsertMessage({
                key: {
                    ...
                    fromMe: false,
                    participant: jidNormalizedUser(authState.creds.me.id)
                },
...
export const extractCommunityMetadata = (result) => {
    ...
    const metadata = {
        ...
        participants: getBinaryNodeChildren(community, 'participant').map(({ attrs }) => {
            ...
            return {
                id: isLidUser(jid) && isPnUser(phoneNumber) ? phoneNumber : jid,
                lid: isPnUser(jid) && isLidUser(lid) ? lid : (isLidUser(jid) ? jid : undefined),
