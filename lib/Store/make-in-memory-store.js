...
export const makeInMemoryStore = (config = {}) => {
    ...
    const contactHashCache = new Map();
    const getContactByHash = async (hash) => {
        if (contactHashCache.has(hash)) {
            const jid = contactHashCache.get(hash);
            return contacts[jid];
        }
        const entries = await Promise.all(
            Object.keys(contacts).map(async (contactId) => {
                const { user } = jidDecode(contactId);
                const h = (await md5(Buffer.from(user + 'WA_ADD_NOTIF', 'utf8'))).toString('base64').slice(0, 3);
                contactHashCache.set(h, contactId);
                return [contactId, h];
            })
        );
        const found = entries.find(([, h]) => h === hash);
        return found ? contacts[found[0]] : undefined;
    };
        if (!messages[jid]) {
    ...
    const bind = (ev) => {
        ...
        ev.on('messaging-history.set', ({ chats: newChats, contacts: newContacts, messages: newMessages, isLatest, syncType }) => {
            if (syncType === WAProto.HistorySync.HistorySyncType.ON_DEMAND) {
                // ON_DEMAND syncs are partial fetches (e.g. load more messages); merge without clearing existing state
        ...
        ev.on('contacts.update', async (updates) => {
            for (const update of updates) {
                ...
                if (contacts[update.id]) {
                    ...
                    contact = await getContactByHash(update.id);
                }
                if (contact) {
        ...
        ev.on('labels.association', ({ type, association }) => {
            switch (type) {
                ...
                case 'remove':
                    ...
                    break;
                default:
                    logger.error({ type }, 'unknown labels.association operation type');
            }
        });
        ...
        ev.on('messages.update', updates => {
            for (const { update, key } of updates) {
                const list = assertMessageList(jidNormalizedUser(key.remoteJidAlt || key.remoteJid));
                if (update?.status) {
        ...
        ev.on('messages.delete', item => {
            if ('all' in item) {
                ...
                const jid = jidNormalizedUser(item.keys[0].remoteJidAlt || item.keys[0].remoteJid);
        ...
        ev.on('message-receipt.update', updates => {
            for (const { key, receipt } of updates) {
                const obj = messages[jidNormalizedUser(key.remoteJidAlt || key.remoteJid)];
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            if (list && mode === 'before' && (!cursorKey || cursorValue)) {
                if (cursorValue) {
                    const msgIdx = list.array.indexOf(cursorValue);
                    msgs = msgIdx >= 0 ? list.array.slice(0, msgIdx) : list.array;
                }
                else {
                    msgs = list.array;
                }
        ...
        mostRecentMessage: async (jid) => messages[jid]?.array.at(-1),
