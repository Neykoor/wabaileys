...
(function silenceLibsignalSessionLogs() {
    const proto = libsignal.SessionRecord?.prototype;
    if (!proto || proto.__erisSilenced) return;
    proto.__erisSilenced = true;
    const CLOSED_SESSIONS_MAX = 40;
    proto.closeSession = function closeSession(session) {
        if (this.isClosed(session))
            return;
        session.indexInfo.closed = Date.now();
    };
    proto.openSession = function openSession(session) {
        session.indexInfo.closed = -1;
    };
    proto.removeOldSessions = function removeOldSessions() {
