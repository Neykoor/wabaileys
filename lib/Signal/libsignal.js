...
;(function _silenceSessionRecordNoise() {
    const proto = libsignal?.SessionRecord?.prototype;
    if (!proto || proto.__wabaileysPatched) return;
    proto.__wabaileysPatched = true;
    ...
    const Ctor = libsignal.SessionRecord;
    if (typeof Ctor?.migrate === 'function') {
        Ctor.migrate = function (data) {
            let run = data.version === undefined;
            const migrateV1 = (d) => {
                const sessions = d._sessions;
                if (!d.registrationId) {
                    for (const key in sessions) {
                        if (sessions[key].indexInfo.closed === -1) {
                            console.error('V1 session storage migration error: registrationId', d.registrationId, 'for open session version', d.version);
                        }
                    }
                }
                else {
                    for (const key in sessions) {
                        if (!sessions[key].registrationId) {
                            sessions[key].registrationId = d.registrationId;
                        }
                    }
                }
            };
            const list = [{ version: 'v1', migrate: migrateV1 }];
            for (let i = 0; i < list.length; ++i) {
                if (run) {
                    list[i].migrate(data);
                }
                else if (list[i].version === data.version) {
                    run = true;
                }
            }
            if (!run) {
...
;(function _suppressSessionNoise() {
    const _isNoise = (...args) => {
        if (typeof args[0] === 'string') {
            const s = args[0];
            if (s.startsWith('Closing session') || s.startsWith('Opening session') ||
                s.startsWith('Closing stale open session') || s.startsWith('Closing open session in favor of') ||
                s.startsWith('Session already closed') || s.startsWith('Session already open') ||
                s.startsWith('Removing old closed session') || s.startsWith('Migrating session to') ||
                s.includes('SessionEntry') || s.includes('pendingPreKey') || s.includes('currentRatchet') ||
    ...
    process.stdout.write = function(chunk, ...rest) {
        if (typeof chunk === 'string' && (
            ...
            chunk.includes('Closing stale open session') || chunk.includes('Closing open session in favor of')
        )) return true;
...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc, lidToPnFunc) {
    const lidMapping = new LIDMappingStore(auth.keys, logger, pnToLIDFunc, lidToPnFunc);
