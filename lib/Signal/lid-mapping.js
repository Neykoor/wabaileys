...
export class LIDMappingStore {
    constructor(keys, logger, pnToLIDFunc, lidToPnFunc) {
    ...
    async _getPNsForLIDsImpl(lids) {
        ...
        // 🛠️ FIX: mapa auxiliar para poder re-decodificar cada lid pendiente
        // después del fallback USync, sin recorrer `lids` de nuevo.
        ...
        for (const lid of lids) {
            ...
            pendingByLidUser.set(lidUser, { lid, decoded });
        }
        if (pending.length) {
            ...
            // 🛠️ FIX: antes, si no había nada en caché ni en el store local, la
            // resolución LID → PN se rendía aquí y `resolve()` devolvía null aunque
            // WhatsApp sí tuviera el dato. Ahora, igual que ya hace la resolución
            // PN → LID, se intenta un fetch USync en vivo como último recurso.
            if (stillMissing.length && this.lidToPnFunc) {
                this.logger.trace(`No local mapping for ${stillMissing.length} LIDs; querying USync`);
                const result = await this.lidToPnFunc(stillMissing).catch((err) => {
                    this.logger.warn({ err }, 'lidToPnFunc USync fetch failed');
                    return [];
                });
                if (result && result.length) {
                    ...
                    for (const pair of result) {
                        ...
                        if (!pnUser)
                            continue;
                        const entry = pendingByLidUser.get(lidUser);
                        if (!entry)
                            continue;
                        this.mappingCache.set(`lid:${lidUser}`, pnUser);
                        this.mappingCache.set(`pn:${pnUser}`, lidUser);
                        addResolvedPair(entry.lid, entry.decoded, pnUser);
                    }
                }
                else {
                    this.logger.trace('USync fetch yielded no results for pending LIDs');
                }
            }
        }
